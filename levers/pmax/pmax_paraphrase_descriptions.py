...
class PMAXDescriptionParaphrase(Lever):

    @Lever.log_generate
    def generate(self) -> None:

        ## Detect Language
        self.input_dict['language'] = detect_language(self.input_dict['reference_description'])
...
Reference Headline: {self.input_dict['reference_description']}
...
Variation 1:'''
    ...
    def postprocess(self) -> None:
        if self.input_dict['language'] == 'English':
            post_process_obj = self.postprocess_class(title_case=False, exclude_exclamation=True)
...
if __name__ == '__main__':
    ...
    ids = {
            ...
            "reference_description" : "Quer milhas, cashback, desconto na fatura",
            "reference_long_headline": "You'll wear these daily."
                    }
    gens, logs = PMAXDescriptionParaphrase().run(ids)
    print(gens)
    # print(logs)
