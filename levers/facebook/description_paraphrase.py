...
class DescriptionParaphrase(Lever):
    ...
    def generate(self) -> None:
        ...
        ## Detect language
        text = " ".join([self.input_dict['reference_description'], input_additional_reference])
        ...
        replacement_instructions, additional_instructions = '', ''
        ...
        self.prompt = [
            {
                "role": "system", 
                "content": f'''You are a helpful digital marketing assistant for writing creative Facebook ad Descriptions for the given Brand Info, Reference Description and Additional Reference
{'Make sure Variations follow the following compliance guidelines.' + replacement_instructions if replacement_instructions else ''} '''},
            ...
            {
                "role": "user", 
                "content": f'''Write creative Facebook ad Descriptions for the given Brand Info, Reference Description and Additional Reference.
...
Reference Description: Free Shipping + Free Returns.
...
Write 6 Variations of the Reference Descriptions using context from the Brand Info and Additional Reference. All Variations must be less than 40 characters.
...
Reference Description: Rethink every meal, and save! 
...
Reference Description: {self.input_dict['reference_description']}
...
Variation 1:'''
    ...
    def postprocess(self) -> None:
        if self.input_dict['language'] == 'English':
            ...
            for generation in self.extracted_generations:
                self.post_process_list.append(post_process_obj.run(generation, self.input_dict, reference_input=self.input_dict['reference_description']))
    ...
    def filter_generations(self) -> None:
        filter_obj = self.filter_generations_class(max_length= max(len(self.input_dict['reference_description']), 50) , min_length=15, threshold=84)
        self.filtered_list, self.log_json['discard_filter_generations'] = \
            filter_obj.run(
                self.post_process_list, 
                reference_ad = self.input_dict['reference_description'],
...
if __name__ == '__main__':
    ...
    id3 = {
        ...
        "reference_primary_text": '''Join healthcare professionals, technology providers and government representatives as we look at the transformation of healthcare across the region.''',
        "reference_headline": 'Manifest transformative health tech at HIMS.',
        "reference_description": 'Get Prescription Fast'.lower(),
        "brand_name" : 'hims',
        'bu_name': 'hims',
        'interest_keyword': 'Mental Health',
        # "brand_id" : "f226bd37-db7c-4908-b992-907ff441bcb7_",
    ...
    for id in ids:
        gens, logs = DescriptionParaphrase().run(id)
        print(gens)
